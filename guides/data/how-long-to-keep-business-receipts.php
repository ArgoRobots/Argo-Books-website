<?php
// guides/data/how-long-to-keep-business-receipts.php
...
<p>Because Argo Books is a desktop app, your records stay on your own machine rather than on someone else's server. Receipt scanning is free for up to {argo_free_receipt_scan_limit} receipts a month, and Premium raises that to {argo_receipt_scan_limit} a month at <strong>${argo_premium_monthly}/month</strong> if you're running higher volume. There's also a free web receipt scanner on the site if you just want to try it on a receipt sitting on your desk right now. Either way, the receipt gets captured once and stays readable for the whole retention window, which is the entire point of keeping it.</p>
HTML,
    ],

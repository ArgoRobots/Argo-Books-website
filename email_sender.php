...
/**
 * Send premium key redemption confirmation email
 *
 * @param string $email User's email address
 * @param string $username Username of the account
 * @param string $key The premium key that was redeemed (will be partially masked)
 * @param string $subscriptionId The newly created subscription ID
 * @param int $durationMonths Duration in months (0 = permanent)
 * @param string $startDate Subscription start date
 * @param string $endDate Subscription end date
 * @return bool Success status
 */
function send_premium_key_redeemed_email($email, $username, $key, $subscriptionId, $durationMonths, $startDate, $endDate)
{
    // Mask the key: PREM-XXXX-XXXX-XXXX-XXXX -> PREM-****-****-****-XXXX
    $maskedKey = substr($key, 0, 5) . '****-****-****-' . substr($key, -4);

    // Format duration text
    $durationText = $durationMonths == 0
        ? 'Permanent'
        : $durationMonths . ' month' . ($durationMonths > 1 ? 's' : '');

    // Format dates
    $formattedStartDate = date('F j, Y', strtotime($startDate));
    $formattedEndDate = $durationMonths == 0
        ? 'Never (permanent)'
    ...
    $body = <<<HTML
        <h1>Premium Key Redeemed</h1>
        <p>Hello {$safeUsername},</p>
        <p>A premium subscription key was just redeemed on your Argo account.</p>

        <table class="details-table">
            <tr>
                <td><strong>Key</strong></td>
                <td class="monospace">{$maskedKey}</td>
            </tr>
            <tr>
                <td><strong>Subscription ID</strong></td>
                <td class="monospace">{$subscriptionId}</td>
            </tr>
            <tr>
        ...
        <div class="info-box info-box-warning">
            <p><strong>Didn't redeem this key?</strong> If you did not perform this action, your account may have been accessed by someone else. Please take these steps immediately:</p>
            <ol>
                <li><a href="https://argorobots.com/community/users/forgot_password.php">Reset your password</a> to secure your account</li>
                <li><a href="https://argorobots.com/contact-us/">Contact our support team</a> so we can revoke the unauthorized subscription and investigate</li>
            </ol>
        </div>
    ...
    return send_styled_email($email, 'Premium Key Redeemed - Argo Books', $body, 'purple');
}
